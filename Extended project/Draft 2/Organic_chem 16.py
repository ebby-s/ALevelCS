...
question_files = {0:'easy questions.txt',1:'med questions.txt',2:'hard questions.txt'}
...
def inst_page(screen):
    ...
    pygame.draw.rect(screen, (255,0,36), start_button)
    lines = split_text(inst_text,760,main_font)
    offset = 0
    for line in lines:
    ...
    for i,pos in enumerate([(120,50),(335,330),(40,60)]):
        screen.blit(texts[i],pos)
...
def diff_select(screen):
    ...
    buttons,button_texts = make_labled_buttons([[220, 165, 800, 120],[220, 355, 800, 120],[220, 545, 800, 120]],["Easy - Answer with one process","Medium - Answer with multiple processes","Hard - Good luck"],title_font)
...
def main_control(screen,diff):
    ...
    while not stop:
        ...
        if question == False:
            warning_screen('The file "'+question_files[diff]+'" cannot be found, please make sure that it exists and is in the same directory.')
            break
...
def generate(diff):
    try:
        i,o,p,allp = load_text(question_files[diff])
    except:
        return False
