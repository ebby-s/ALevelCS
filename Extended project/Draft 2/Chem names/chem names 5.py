...
def Draw(startpos,angle,length,distance=50):
    startpos[1] += 8
    pen.setpos(startpos)
    pen.seth(angle)
    pen.pendown()
...
for i in final["groups"]:
    if int(i[0])%2 == 0 and i[0] not in takenpos:
        direct = 60
        side = 25
    elif int(i[0])%2 == 1 and i[0] in takenpos:
        direct = 60
        side = 0
    elif int(i[0])%2 == 1 and i[0] not in takenpos:
        direct = 240
        side = 0
    elif int(i[0])%2 == 0 and i[0] in takenpos:
        direct = 240
        side = 25
