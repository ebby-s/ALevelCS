...
def main_control(screen,diff):
    ...
    while not stop:
        ...
        if question == False:
            warning_screen('The file "'+question_files[diff]+'" either cannot be found or is empty, please make sure that it exists, is in the same directory and has content.')
            break
...
def generate(diff):
    try:
        i,o,p,allp = load_text(level_files[diff])
        if len(i) == 0:
            return False
    except:
        return False
...
[levels,level_descriptions,level_files] = [settings[0],settings[1],settings[3]]
